...
#   """
#   with ops.name_scope(name, "Shape", [input]) as name:k
...
def one_hot(indices,
  ...
  def forward(indices, depth):
    ...
    if isinstance(indices, int):
      result_shape = [depth]
    elif isinstance(indices, ops.Tensor):
      result_shape = indices.shape + [depth]
    else:
      raise NotImplementedError("This is the type of indices:{}".format(type(indices)))
    return ops.Tensor(result_shape)

  this_operation = ops.our_Operation([indices, depth], ffnc=forward, name="one_hot")   # create a new operation object each time
  gph = ops.our_Graph.get_default_graph()
  gph.operations.append(this_operation)
  return this_operation
  
  # with ops.name_scope(name, "one_hot", [indices, depth, on_value, off_value, axis, dtype]) as name:
  #   on_exists = on_value is not None
  #   off_exists = off_value is not None

  #   on_dtype = (ops.convert_to_tensor(on_value).dtype.base_dtype if on_exists else None)
  #   off_dtype = (ops.convert_to_tensor(off_value).dtype.base_dtype if off_exists else None)
  ...
  #     raise TypeError("dtype {0} of on_value does not match dtype {1} of off_value".format(on_dtype, off_dtype))
