...
def maxPool(img, k, s):
    ...
    '''
    ret = tf.nn.max_pool(img, ksize=[1, k, k, 1], strides=[1, s, s, 1], padding='SAME')
    return ret

def norm(layer, lsize):
    ret = tf.nn.lrn(layer, lsize, bias=2.0, alpha=0.001 / 9.0, beta=0.75)
    return ret
...
def convolutionalNet(img, _weights, _biases, _dropout) :
    ...
    wd1Shape = _weights['wd1'].get_shape()      # for TF light
...
def main():
    # Build the model
    z = tf.placeholder(tf.float32, shape=[None, N_INPUT])
    y = tf.placeholder(tf.float32, shape=[None, N_CLASSES])
    keepProb = tf.placeholder(tf.float32) # dropout (keep probability)
    pred = buildModel(z, keepProb)
    ...
    with tf.Session() as sess :
        ...
        for batch in batches :
            ...
            ans = sess.run(softmax, feed_dict={z: batchImages, y: batchLabels, keepProb: P_DROPOUT})
